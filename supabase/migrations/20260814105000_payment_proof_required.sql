-- A manually reviewed payment claim is not approvable without its proof.
-- Enforce the requirement in the RPC, not only in the Flutter form.
...
begin
  ...
  if not exists (
    select 1 from public.profiles p where p.id = v_uid and p.role = 'contractor'
  ) then
  ...
  if p_proof_path is null or p_proof_path !~ ('^' || v_uid::text || '/[A-Za-z0-9._/-]+$') then
