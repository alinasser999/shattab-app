-- Discovery pages must not move under an active catalogue. Offset pagination
-- duplicates or skips professionals when a profile changes while a homeowner
-- scrolls, so the paged endpoints use the exact tuple they sort by.
